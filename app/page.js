        ...
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button
              ...
              disabled={scanningSocial}
              className={`px-4 py-2 text-sm rounded-lg transition ${scanningSocial ? "bg-pink-400 text-white cursor-wait" : "bg-pink-50 text-pink-700 border border-pink-200 hover:bg-pink-100"}`}>
              {scanningSocial ? "Scanner SoMe..." : "\u{1F4F1} SoMe-scan"}
            </button>
            <button
              onClick={runScan}
              disabled={scanning}
              className={`px-4 py-2 text-white text-sm rounded-lg transition ${scanning ? "bg-blue-400 cursor-wait" : "bg-blue-600 hover:bg-blue-700"}`}>
        ...
        <div className="flex gap-2 mb-3 flex-wrap">
          {CATEGORIES.map(c => (
            <button key={c} onClick={() => setFilter(c)}
              className={`px-3 py-1.5 text-xs rounded-full border transition ${filter === c ? "bg-blue-50 text-blue-700 border-blue-200" : "bg-white text-gray-600 border-gray-200 hover:bg-gray-50"}`}>
        ...
        <div className="flex gap-2 mb-5 flex-wrap">
          {SOURCES.map(s => (
            <button key={s} onClick={() => setSourceFilter(s)}
              className={`px-3 py-1.5 text-xs rounded-full border transition ${sourceFilter === s ? "bg-purple-50 text-purple-700 border-purple-200" : "bg-white text-gray-600 border-gray-200 hover:bg-gray-50"}`}>
