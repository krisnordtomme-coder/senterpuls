...
function extractImages($, pageUrl) {
    ...
    $("article img, .product img, .campaign img, .hero img, main img").each((_, el) => {
...
function extractContent(html, store, pageUrl) {
  ...
  const metaDesc = $('meta[name="description"]').attr("content") || ""
    ...
    if (metaText && metaText.length > 30) {
          blocks.push({ text: metaText, url: pageUrl, images: pageImages })
    }
        ...
        try {
                ...
                for (const item of items) {
                          if (item["@type"] === "Product" && item.offers) {
                                      ...
                                      const productImages = item.image ? (Array.isArray(item.image) ? item.image : [item.image]) : []
                                                  if (text.length > 30) blocks.push({ text: text.substring(0, 400), url: pageUrl, images: productImages.length > 0 ? productImages : pageImages })
                          }
                          if (item["@type"] === "Event") {
                                      const text = [item.name, item.description, item.startDate].filter(Boolean).join(" - ")
                                      const eventImages = item.image ? (Array.isArray(item.image) ? item.image : [item.image]) : []
                                                  if (text.length > 20) blocks.push({ text: text.substring(0, 400), url: pageUrl, images: eventImages.length > 0 ? eventImages : pageImages })
                          }
                }
        } catch {}
  })
  ...
  $("script, style, nav, footer, iframe, noscript, svg, form").remove()
    ...
    $("h1, h2, h3, h4, p, li, a").each((_, el) => {
          ...
          if (text.length > 25 && text.length < 500 && rx.test(text)) {
                  const closestImg = $(el).closest("article, section, div").find("img").first().attr("src")
                  const blockImages = closestImg ? [closestImg.startsWith("http") ? closestImg : new URL(closestImg, pageUrl).href] : pageImages
                  blocks.push({ text, url: pageUrl, images: blockImages })
          }
    })
  ...
  $("h1, h2").each((_, el) => {
        ...
        if (text.length > 15 && text.length < 200) {
                blocks.push({ text: "[Hovedbudskap] " + text, url: pageUrl, images: pageImages })
        }
  })
...
async function scrapeStore(store) {
    ...
    if (html) results.push(...extractContent(html, store, store.url))
  ...
  const baseUrl = store.url.replace(/\/+$/, "")
    for (const path of CAMPAIGN_PATHS) {
          ...
          if (subHtml && subHtml.length > 1000) results.push(...extractContent(subHtml, store, subUrl))
    }
            ...
            for (let j = 0; j < batch.length; j++) {
                      ...
                      for (const block of results[j]) {
                        ...
                        const { error } = await supabase.from("content").insert({
                                      ...
                                      image_urls: block.images?.filter(img => img?.startsWith("http")) || [],
                                      content_hash: hash,
                        })
