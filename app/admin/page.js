    ...
    try {
      ...
      if (error) {
        ...
        if (mData) {
          ...
          setTeamMembers(mData.map(m => ({ ...m, profiles: pData?.find(p => p.id === m.user_id) || null })))
          return
        }
      }
      setTeamMembers(data || [])
    ...
    <div style={{ minHeight: "100vh", background: "#FAE4FB" }}>
      <header style={{ background: "#121226", color: "white", padding: "1rem 2rem", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
          ...
          <button onClick={signOut} style={{ background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.2)", color: "white", padding: "0.4rem 0.8rem", borderRadius: "8px", cursor: "pointer", fontSize: "0.8rem" }}>Logg ut</button>
        </div>
      </header>
      ...
      <div style={{ maxWidth: "1100px", margin: "0 auto", padding: "2rem" }}>
        ...
        {showNewOrg && (
          <div style={{ background: "white", borderRadius: "20px", padding: "2rem", marginBottom: "1.5rem", boxShadow: "0 4px 20px rgba(0,0,0,0.06)" }}>
            ...
            {error && <p style={{ color: "#991b1b", fontSize: "0.85rem", background: "#fee2e2", padding: "0.5rem 1rem", borderRadius: "8px" }}>{error}</p>}
            <input value={newOrgName} onChange={e => setNewOrgName(e.target.value)} placeholder="Organisasjonsnavn (f.eks. Carucel Eiendom)" style={{ width: "100%", padding: "0.75rem 1rem", border: "2px solid #E7E1E3", borderRadius: "12px", fontSize: "0.95rem", boxSizing: "border-box", marginBottom: "1rem" }} />
            <div style={{ display: "flex", gap: "0.5rem" }}>
              <button onClick={createOrganization} disabled={saving} style={{ background: "#D4FF66", color: "#121226", border: "none", padding: "0.6rem 1.5rem", borderRadius: "10px", fontWeight: 600, cursor: "pointer" }}>{saving ? "Oppretter..." : "Opprett"}</button>
              <button onClick={() => { setShowNewOrg(false); setError(null) }} style={{ background: "#E7E1E3", color: "#360817", border: "none", padding: "0.6rem 1.5rem", borderRadius: "10px", cursor: "pointer" }}>Avbryt</button>
            </div>
          </div>
        )}
        ...
        {currentOrg && (
          <>
            <div style={{ display: "flex", gap: "0.5rem", marginBottom: "1.5rem" }}>
              ...
              <div style={{ flex: 1 }} />
              <button onClick={() => setShowNewOrg(true)} style={{ background: "white", color: "#360817", border: "none", padding: "0.7rem 1.2rem", borderRadius: "12px", cursor: "pointer", fontSize: "0.85rem" }}>+ Ny org</button>
            </div>
            ...
            {tab === "centers" && (
              <div style={{ background: "white", borderRadius: "20px", padding: "2rem", boxShadow: "0 4px 20px rgba(0,0,0,0.06)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem" }}>
                  ...
                  {isAdmin && <button onClick={() => setShowNewCenter(true)} style={{ background: "#D4FF66", color: "#121226", border: "none", padding: "0.6rem 1.2rem", borderRadius: "10px", fontWeight: 600, cursor: "pointer" }}>+ Nytt senter</button>}
                </div>
                {showNewCenter && (
                  <div style={{ background: "#FAE4FB", borderRadius: "14px", padding: "1.5rem", marginBottom: "1.5rem" }}>
                    <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap" }}>
                      <input value={newCenterName} onChange={e => setNewCenterName(e.target.value)} placeholder="Senternavn" style={{ flex: 2, minWidth: "200px", padding: "0.65rem 1rem", border: "2px solid #E7E1E3", borderRadius: "10px", fontSize: "0.9rem" }} />
                      <input value={newCenterCity} onChange={e => setNewCenterCity(e.target.value)} placeholder="By" style={{ flex: 1, minWidth: "120px", padding: "0.65rem 1rem", border: "2px solid #E7E1E3", borderRadius: "10px", fontSize: "0.9rem" }} />
                      <button onClick={createCenter} disabled={saving} style={{ background: "#121226", color: "white", border: "none", padding: "0.65rem 1.2rem", borderRadius: "10px", fontWeight: 600, cursor: "pointer" }}>{saving ? "..." : "Opprett"}</button>
                      <button onClick={() => setShowNewCenter(false)} style={{ background: "#E7E1E3", color: "#360817", border: "none", padding: "0.65rem 1rem", borderRadius: "10px", cursor: "pointer" }}>Avbryt</button>
                    </div>
                  </div>
                )}
                {centers.length === 0 ? (
                  ...
                  <div style={{ display: "grid", gap: "0.75rem" }}>
                    {centers.map(c => (
                      <div key={c.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "1rem 1.25rem", borderRadius: "14px", background: c.active ? "#f8f7ff" : "#f5f5f5", border: "1px solid #E7E1E3" }}>
                        <div>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
                          <span style={{ fontSize: "0.75rem", padding: "0.2rem 0.6rem", borderRadius: "6px", background: c.active ? "#D4FF66" : "#E7E1E3", color: "#360817" }}>{c.active ? "Aktiv" : "Inaktiv"}</span>
                          {isAdmin && <button onClick={() => toggleCenterActive(c)} style={{ background: "none", border: "1px solid #E7E1E3", borderRadius: "8px", padding: "0.3rem 0.6rem", cursor: "pointer", fontSize: "0.75rem", color: "#360817" }}>{c.active ? "Deaktiver" : "Aktiver"}</button>}
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            {tab === "team" && (
              <div style={{ background: "white", borderRadius: "20px", padding: "2rem", boxShadow: "0 4px 20px rgba(0,0,0,0.06)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem" }}>
                  ...
                  {isAdmin && <button onClick={() => setShowInvite(true)} style={{ background: "#D4FF66", color: "#121226", border: "none", padding: "0.6rem 1.2rem", borderRadius: "10px", fontWeight: 600, cursor: "pointer" }}>+ Inviter</button>}
                </div>
                <div style={{ display: "grid", gap: "0.75rem" }}>
                  {teamMembers.map(m => (
                    <div key={m.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "1rem 1.25rem", borderRadius: "14px", background: "#f8f7ff", border: "1px solid #E7E1E3" }}>
                      <div>
                      ...
                      <span style={{ fontSize: "0.75rem", padding: "0.25rem 0.7rem", borderRadius: "8px", background: roleColors[m.role] || "#E7E1E3", color: "#360817", fontWeight: 600 }}>{roleLabels[m.role] || m.role}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {tab === "invitations" && (
              <div style={{ background: "white", borderRadius: "20px", padding: "2rem", boxShadow: "0 4px 20px rgba(0,0,0,0.06)" }}>
                  ...
                  <div style={{ background: "#FAE4FB", borderRadius: "14px", padding: "1.5rem", marginBottom: "1.5rem" }}>
                    <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap", alignItems: "flex-end" }}>
                      <div style={{ flex: 2, minWidth: "200px" }}>
                        ...
                        <input value={inviteEmail} onChange={e => setInviteEmail(e.target.value)} placeholder="bruker@epost.no" type="email" style={{ width: "100%", padding: "0.65rem 1rem", border: "2px solid #E7E1E3", borderRadius: "10px", fontSize: "0.9rem", boxSizing: "border-box" }} />
                      </div>
                      ...
                      <button onClick={sendInvitation} disabled={saving} style={{ background: "#121226", color: "white", border: "none", padding: "0.65rem 1.2rem", borderRadius: "10px", fontWeight: 600, cursor: "pointer" }}>Send</button>
                      <button onClick={() => setShowInvite(false)} style={{ background: "#E7E1E3", color: "#360817", border: "none", padding: "0.65rem 1rem", borderRadius: "10px", cursor: "pointer" }}>Avbryt</button>
                    </div>
                  </div>
                )}
                {invitations.length === 0 ? (
                  ...
                  <div style={{ display: "grid", gap: "0.75rem" }}>
                    {invitations.map(inv => (
                      <div key={inv.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "1rem 1.25rem", borderRadius: "14px", background: "#f8f7ff", border: "1px solid #E7E1E3" }}>
                        <div>
              ...
              <div style={{ background: "white", borderRadius: "20px", padding: "2rem", boxShadow: "0 4px 20px rgba(0,0,0,0.06)" }}>
                ...
                <div style={{ display: "grid", gap: "1rem" }}>
                  <div>
                    ...
                    <input value={currentOrg.name} readOnly style={{ width: "100%", padding: "0.75rem 1rem", border: "2px solid #E7E1E3", borderRadius: "12px", fontSize: "0.95rem", background: "#f8f7ff", boxSizing: "border-box" }} />
                  </div>
                  <div>
                    <label style={{ display: "block", fontSize: "0.85rem", color: "#360817", marginBottom: "0.3rem", fontWeight: 500 }}>Slug</label>
                    <input value={currentOrg.slug} readOnly style={{ width: "100%", padding: "0.75rem 1rem", border: "2px solid #E7E1E3", borderRadius: "12px", fontSize: "0.95rem", background: "#f8f7ff", boxSizing: "border-box" }} />
                  </div>
