...
export async function supabaseDirectUpdate(table, match, patch) {
  const response = await fetch(`${supabaseUrl}/rest/v1/${table}?${eqParams(match)}`, {
    method: "PATCH",
...
export async function supabaseDirectInsert(table, rows) {
...
// DELETE rows filtered by `match` (column -> value). Returns the deleted rows.
export async function supabaseDirectDelete(table, match) {
  const response = await fetch(`${supabaseUrl}/rest/v1/${table}?${eqParams(match)}`, {
    method: "DELETE",
    headers: authHeaders(getAccessToken())
  })
